...
<body style='font-family:Arial,sans-serif;'>
    <div style='border:1px solid #ccc; padding:20px; border-radius:10px; max-width:600px; margin:auto;'>
        ...
        public static string OrderInvoice(Order order, string senderEmail, string qrCid = "")
        {
            ...
            foreach (var d in order.OrderDetails)
            {
                ...
                if (d.ContainerTypeNavigation != null)
                    parts.Add($"Thùng {d.ContainerTypeNavigation.Type}");

                if (d.StorageTypeNavigation != null)
                    parts.Add($"Kho {d.StorageTypeNavigation.Name}");

                if (d.ShelfTypeNavigation != null)
                    parts.Add($"Kệ {d.ShelfTypeNavigation.Name}");
                ...
                var quantity =
                    d.Quantity ??
                    d.ContainerQuantity?.ToString() ??
                    d.ShelfQuantity?.ToString() ??
                    "1";
        ...
        ");
            ...
            return $@"
                    ...
                    <body style='font-family:Arial, Helvetica, sans-serif; background:#fff;'>
                        <div style='max-width:800px; margin:auto; padding:30px; border:1px solid #ddd;'>

                            <h2 style='text-align:center; margin-bottom:0;'>PHIẾU ĐƠN HÀNG</h2>
                            <p style='text-align:center; margin-top:5px; color:#666;'>ASMS</p>

                            <table style='width:100%; margin-top:25px;'>
                                <tr>
                                    <td>
                                        <b>Mã đơn:</b> {order.OrderCode}<br/>
                                        <b>Khách hàng:</b> {order.CustomerName}<br/>
                                        <b>Địa chỉ:</b> {order.Address}
                                    </td>
                                    <td style='text-align:right;'>
                                        <b>Trạng thái:</b> {order.PaymentStatus}<br/>
                                        <b>Ngày:</b> {order.OrderDate?.ToString("dd.MM.yyyy")}
                                    </td>
                                </tr>
                            </table>
                            ...
                            <p style='text-align:right; margin-top:15px; font-size:16px;'>
                                <b>Tổng cộng: {order.TotalPrice:N0} đ</b>
                            </p>

                            {(order.UnpaidAmount > 0
                                        ? $"<p style='text-align:right;'>Còn nợ: <b>{order.UnpaidAmount:N0} đ</b></p>"
                                        : "")}

                            {(string.IsNullOrEmpty(qrCid) ? "" : $@"
                    ...
                    />
                                ...
                                <p style='font-size:12px; color:#666;'>Quét QR để xem thông tin đơn hàng</p>
                            </div>
                            ")}
